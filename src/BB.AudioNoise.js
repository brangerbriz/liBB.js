...
function(  BB,        Audio,        Detect ){
	...
	BB.AudioNoise = function( config ) {
		...
		if( BB.Audio.context instanceof Array ){
			if( typeof config === "undefined" || typeof config.context === "undefined" )
				throw new Error('BB.AudioNoise: BB.Audio.context is an Array, specify which { context:BB.Audio.context[?] }');
			else 
		...
		if( typeof config.connect !== 'undefined' ){
			if( config.connect instanceof AudioDestinationNode || config.connect instanceof AudioNode ) 
				...
				throw new Error('BB.AudioNoise: connect property expecting an AudioNode');
	...
	* @default 1
	*/   
	Object.defineProperty(BB.AudioNoise.prototype, "volume", {
		get: function() {
			return this._volume;
		},
		set: function(v) {
			if( typeof v !== 'number'){
				throw new Error("BB.AudioNoise.volume: expecing a number");
			} else {
				this.setGain( v );
			}
		}
	...
	*  &nbsp;});<br>
	*  <br>
	*  &nbsp; whitenoise.connect( exampleNode );<br>
	...
	*  &nbsp; // ...then you've got whitenoise connected to BB.Audio.context twice<br>
	* </code>
	* <br>
	...
	*/
	BB.AudioNoise.prototype.connect = function( destination, output, input ){
		if( !(destination instanceof AudioDestinationNode || destination instanceof AudioNode) )
			throw new Error('BB.AudioNoise.connect: destination should be an instanceof AudioDestinationNode or AudioNode');
		if( typeof output !== "undefined" && typeof output !== "number" )
			throw new Error('BB.AudioNoise.connect: output should be a number');
		if( typeof intput !== "undefined" && typeof input !== "number" )
			throw new Error('BB.AudioNoise.connect: input should be a number');
	...
	*  &nbsp;});<br>
	*  <br>
	*  &nbsp;whitenoise.disconnect(); // disconnected from default BB.Audio.context<br>
	*  &nbsp;whitenoise.connect( exampleNode ); // connected to exampleNode only<br>
	*  &nbsp;whitenoise.makeNote('A');<br>
	* </code>
	* <br>
	* ...which looks like this ( where the first Gain is the Noise and the second is the exampleNode )<br>
	* <img src="../assets/images/audiosampler4.png">
	*/
	BB.AudioNoise.prototype.disconnect = function(destination, output, input ){
		if( typeof destination !== "undefined" &&
			!(destination instanceof AudioDestinationNode || destination instanceof AudioNode) )
				throw new Error('BB.AudioNoise.disconnect: destination should be an instanceof AudioDestinationNode or AudioNode');
		if( typeof output !== "undefined" && typeof output !== "number" )
			throw new Error('BB.AudioNoise.disconnect: output should be a number');
		if( typeof input !== "undefined" && typeof input !== "number" )
			throw new Error('BB.AudioNoise.disconnect: input should be a number');
	...
	/**
	* sets the gain level of the Noise ( in a sense, master volume control )
	* @method setGain
	...
	*  &nbsp;});<br>
	*  <br>
	*  &nbsp; whitenoise.setGain( 0.25, 2 ); // lower's volume from 0.75 to 0.25 in 2 seconds <br>
	*  // if no ramp value is needed, you could alternatively do<br>
	*  &nbsp; whitenoise.volume = 0.5; // immediately jumps from 0.25 to 0.5 <br>
	* </code>
	*/
	BB.AudioNoise.prototype.setGain = function( num, gradually ){
		if( typeof num !== "number" )
			throw new Error('BB.AudioNoise.setGain: first argument expecting a number');
		
		this._volume = num;

		if(typeof gradually !== "undefined"){
			if( typeof num !== "number" )
				throw new Error('BB.AudioNoise.setGain: second argument expecting a number');
			else
	...
	BB.AudioNoise.prototype._sec2tc = function( sec ){
		return ( sec * 2 ) / 10;
	};

	BB.AudioNoise.prototype._globalGainUpdate = function( gradually ){
